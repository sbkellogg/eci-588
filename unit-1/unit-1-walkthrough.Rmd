...
`Resource` type which can be accomplished in a couples steps.
...
This let's us assume a good deal of prior knowledge on their end about
...
2.  **Word Search**. I also think it's important to include a some
    actual participant responses for select key words in order to
    provide additional information about how those words are being used
    on context. I may handpick some select quotes or present a random
    selection of quotes.
...
basic slides show that includes the following charts:
...
opd_counts %>%
  ...
  ggplot(aes(n, word)) +
  geom_col() + 
  labs(x = "Word Counts", y = "", title = "20 Most Common Words Used to Describe the Value of Online Learning Resources") + 
  theme_minimal()
...
Then I'll use the `filter()` function to remove any response pertaining to Calendar. 
