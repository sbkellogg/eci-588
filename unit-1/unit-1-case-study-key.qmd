...
Now let's append the `total` column from `teacher_sum` to our `teacher_word` data frame:
...
I use wordclouds pretty sparingly in articles or reports, but typically include them for open ended items in online Qualtrics survey reports to provide education partners I work with a quick snapshot of the response.
...
teacher_percentages |>
  ...
  ungroup() |>
  ggplot(aes(percent, fct_reorder(word, percent), fill = Resource)) +
