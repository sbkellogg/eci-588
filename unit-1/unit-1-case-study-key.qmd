...
Note that the first argument is the data frame you imported earlier and the second argument is the file name you plan to give it, including (if necessary) the file path for where it should go.
...
To begin, we'll `select()` the columns `Role`, `Resource`, and `Q21` columns since those respectively pertain to educator role, OPD resource they are evaluating, and, as illustrated by the second row of column Q21, their response to the survey question, "What was the most beneficial/ aspect of the professional development resource?"

We'll also assigne these selected columns to a new data frame named `opd_selected` by using the `<-` assignment operator.
...
Notice that, like the bulk of tidyverse functions we'll be using throughout our case studies, the first argument that the `select()` fuction expects is a data frame, which in our case is `opd_survey`. The select() function the expects the specific columns you'd like to select.
...
In the code chunk below, use the pipe operator to add the `anti_join()` function to our chain of data wrangling functions as our final text preprocessing step. Also, add a short comment to each line of code explaining each data wrangling step. The first had been done for you.

```{r}
opd_clean <- opd_survey |>
...
Now complete and run the following code tidy your `opd_survey` data and save to a new `opd_benefits` data frame by adding functions for out data wrangling pipeline for tokenizing your text and removing stop words.
...
1.  Why do you think the console provided the message "Joining, by = 'word'"?
    -   YOUR RESPONSE HERE
...
One common approach to facilitate comparison of responses across Online Resource type is by looking at the **frequency** that each word occurs among all words for that document group. This also helps to better gauge how prominent the same word is across different groups.
...
Notice that idf and thus tf-idf are zero for these extremely common words (typically stop words). These are all words that appear in teacher responses for all online resource types, so the idf term is zero. The inverse document frequency (and thus tf-idf) is very low (near zero) for words that occur in many of the documents in a collection; this is how this approach decreases the weight for common words.
...
While some of these words make sense for each resource, like "reflection" for Online Learning Modules or "interface" for Calendar, it's obvious many of these words were only identified as unique simply because they were misspelled words. Clearly something we would attend to for a more rigorous analysis.

::: callout-caution
While it has proved useful in text mining, search engines, etc., its theoretical foundations are considered less than firm by information theory experts..."
