---
title: "Unit 2 Case Study: Twitter Sentiment & School Reform"
...
c.  **Twitter Setup** We case study the process of setting up R to pull data from our Twitter developer account created during the first week of the course.
...
For this case study, we'll use a similar approach used by the authors to guage public sentiment around the NGSS, by compare how much more positive or negative NGSS tweets are relative to CSSS tweets.
...
At the end of this week, I'll ask that you share with me your r script as evidence that you have complete the case study. Although I highly recommend that that you manually type the code shared throughout this case study, for large blocks of text it may be easier to copy and paste.
...
Since one of our goals for this case study is a very crude replication of the study by Rosenberg et al. (2021), let's begin by introducing the `search_tweets()` function to try reading into R 5,000 tweets containing the NGSS hashtag and store as a new data frame `ngss_all_tweets`.
...
**Note:** If you have already created these data frames from 2a. Import Tweets, you do not need to read these file into R unless you want to reproduce the exact same outputs shown in the rest of this case study.
